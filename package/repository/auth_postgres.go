...
func (repo *AuthPostgres) CreateUser(user model.User, logrus *logrus.Logger) (int, error) {
	...
	query := fmt.Sprintf("INSERT INTO %s (email, password, username, city, phone) values ($1, $2, $3, $4, $5) RETURNING id", usersTable)

	row := repo.db.QueryRow(query, user.Email, user.Password, user.UserName, user.City, user.Phone)
...
func (repo *AuthPostgres) VerifyEmail(id int, logrus *logrus.Logger) (int64, error) {
